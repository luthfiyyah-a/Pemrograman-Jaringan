...
class ChatClient:
    ...
    def proses(self,cmdline):
        ...
        try:
            ...
            if (command=='auth'):
                ...
                return self.add_realm(real_id, target_realm_address, target_realm_port)
                ...
                realm_id = j[1].strip()
                return self.get_realm_message(realm_id, group_usernames, message)        
            else:
    ...
    def send_realm_message(self, realm_id, username_to,message):
        if (self.tokenid==""):
            return "Error, not authorized"
        string="send {} {} {} \r\n" . format(self.tokenid,realm_id, sername_to,message)
        print(string)
        result = self.sendstring(string)
        if result['status']=='OK':
            return "message sent to {}" . format(usernameto)
    ...
    def get_realm_inbox(self, realm_id):
        if (self.tokenid == ""):
            return "Error, not authorized"
        string = "getrealminbox {} {} \n\r" . format(self.tokenid, realm_id)
